...
def _make_agent_trace_question(instructions: str) -> dict:
    """Build the agent-trace `typed-decisions` question set.

    Per §5 of the laya doc, tool-call results are evaluated with a single
    multi-question checkpoint whose `state` field lists each sub-question in a
    ``question`` / ``type`` pair (noul = yes/no). The model then returns one
    `answers` entry per id: action, outcome, risk, needs_review, urgency.
    """
    question = {
        "type": "typed-decisions",
        "instructions": instructions,
        "state": [
            {"question": "What is the single-line ACTION laya recommends for this tool result?",
             "type": "noul"},
            {"question": "Outcome: what is the expected result of following this action?",
             "type": "noul"},
            {"question": "Risk: list the key risks (brief).",
             "type": "noul"},
            {"question": "Does this decision need human review, or can it be safely automated?",
             "type": "noul"},
            {"question": "How urgent is deciding on this? Provide a value 0-1.",
             "type": "noul"},
        ],
    }
    return {"agent_trace": question}
...
def _decision_from_agent_trace(result: dict) -> str | None:
    ...
    action = _decision_from_answer(answers.get("action"))
    outcome = extract_answer(answers.get("outcome")).strip()
    risk = extract_answer(answers.get("risk")).strip()
    needs_review = _decision_from_answer(answers.get("needs_review")).lower().lstrip(": ") if answers.get("needs_review") else ""
    urgency = (extract_answer(answers.get("urgency")).strip() or "").split()[0] if answers.get("urgency") else ""

    lines = [f"laya agent-trace: action={action}"]
    if outcome:
