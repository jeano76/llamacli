...
def _emit_fastcheck_sc(result: dict, cfg: dict) -> int:
    ...
    stdout line 1 is exactly "SHORTCIRCUIT" so Node's /^short-circuit$/i on
    stdout.split(/\\r?\\n/)[0] matches; subsequent lines mirror cmd_gate()'s
    System-2 short-circuit output. The allow/deny banner is chosen with the
    exact same wording as cmd_gate() so both paths look identical to a reader.
    """
