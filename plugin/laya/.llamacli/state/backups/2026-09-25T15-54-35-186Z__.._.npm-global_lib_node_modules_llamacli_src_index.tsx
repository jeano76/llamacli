...
async function main() {
  ...
  const runLayaScript = (args: string[]): Promise<{ stdout: string }> =>
    new Promise((resolve, reject) => {
      ...
      child.stdout?.on("data", (d) => resolve({ stdout: String(d) }));
      child.on("exit", (code) => {
        if (!settled) {
          settled = true;
          clearTimeout(timer);
          ...
          code === 0 ? resolve({ stdout: "" }) : reject(new Error(`laya script exited with code ${code}`));
        }
      });
    });
  ...
  const runLayaGate = async (userText: string): Promise<void> => {
    ...
    try {
      ...
      // Success path only: the script has already surfaced laya's verdict via
      // its own stdout capture. Nothing more to push here — see runLayaScript.
    } catch {
