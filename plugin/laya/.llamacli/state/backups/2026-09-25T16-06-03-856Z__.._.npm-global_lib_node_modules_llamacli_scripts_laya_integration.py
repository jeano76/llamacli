...
def main(argv=None) -> int:
    ...
    p_enable = sub.add_parser("enable", help="Enable the fast-check gate.")
    p_disable = sub.add_parser("disable", help="Disable the fast-check gate (immediate no-op).")
    p_status = sub.add_parser("status", help="Show enabled state + integration health.")

    p_fc = sub.add_parser("fastcheck", help="Run the gate against a piece of user text.")
    p_fc.add_argument("--text", required=True, metavar='"user text"',
                      help='The exact user message to evaluate.')
    p_fc.add_argument("--config", help="Override config.yaml path (tests).")
    sub.add_argument("--config", help="Override config.yaml path (tests/global).")
    ...
    try:
        ...
        if args.command == "fastcheck":
            return cmd_fastcheck(path, getattr(args, "text", ""))
