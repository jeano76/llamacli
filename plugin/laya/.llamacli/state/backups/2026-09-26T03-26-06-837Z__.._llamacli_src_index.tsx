...
async function main() {
  ...
  /** Verdicts Python's short_circuit_verdict() treats as "simple enough to skip
   *  System 2" (kept in sync with laya_integration.py). */
  const SHORT_CIRCUIT_ACTIONS = new Set(["yes", "no", "proceed", "flag", "no_action"]);
  ...
  const runLayaGate = async (userText: string): Promise<{ skip: boolean }> => {
    ...
    try {
      ...
      // The script prints "[revised] <action>" to stdout when laya has a verdict.
      // A leading "proceed"/"flag"/"no_action" (or any allow-listed action) is a
      // short-circuit verdict: the task can be answered directly, skipping Ornith.
      const match = stdout.trim().match(/^\[revised\]\s+([a-zA-Z]+)/i);
      if (match && SHORT_CIRCUIT_ACTIONS.has(match[1].toLowerCase())) {
        return { skip: true };
      }
      return { skip: false };                        // no short-circuit verdict => proceed
    } catch {
