...
def resource_gate_ok(cfg: dict) -> tuple[bool, str]:
    ...
    Gate only when BOTH RAM and swap are below threshold: if RAM itself has
    headroom, laya is very unlikely to touch swap at all, so a lone swap
    shortage must not block it. The genuinely dangerous case is "RAM short
    AND no spare swap to absorb from". A RAM shortage alone (swap fine or not)
    is still always blocked — that's the real risk.
    """
    ...
    if ram_short and swap_short:
        return False, (
            f"free RAM {free_ram // 1024}kB and free swap {free_swap // 1024}kB "
