...
import { execFileAsync as _execFileAsync } from "node:child_process";
...
async function main() {
  ...
  const LAYA_TIMEOUT_MS = (config.laya?.timeoutSeconds ?? SYSTEMONE_TIMEOUT) * 1000;
  const layaScriptPath = pathResolve(projectRoot, "scripts/laya_integration.py");

  // Spawn the laya integration script. All config writes, server boot and
  // health checks live in Python; Node only runs it with a bounded timeout so a
  // hung script can never wedge the TUI.
  const runLayaScript = (args: string[]): Promise<{ stdout: string }> =>
    new Promise((resolve, reject) => {
      _execFileAsync(
        "python3",
        [layaScriptPath, ...args],
        { timeout: SYSTEMONE_TIMEOUT * 1000 },
        (err, stdout) => (err ? reject(err) : resolve({ stdout: String(stdout ?? "") }))
      );
    });

  const runLayaGate = async (): Promise<void> => {
    if (!runtimeEnabled) return;              // off => instant no-op (no checks)
    try {
      const result: any = await execFileAsync(
        "python3",
        [layaScriptPath, "fastcheck", "--text", "<turn about to run>"],
        { timeout: LAYA_TIMEOUT_MS }
      );
      // Success path only: surface the System-1 verdict so the model sees it
      // before producing its own turn. stdout already carries laya's prose.
      (globalThis as any).__llamacli_ui?.pushStatus(result.stdout.trim());
    } catch {
      // Any failure (nonzero exit, timeout, script error) => silent fall back:
          ...
          case "fastcheck": {
            ...
            try {
              if (sub === "on" || sub === "enable") {
                await runLayaScript(["enable", rawText]);
                ...
                await runLayaScript(["status", rawText]);
              } else {
                // ad-hoc question: always works regardless of on/off state
                await runLayaScript(["fastcheck", "--text", rawText]);
              }
            } catch (err: any) {
