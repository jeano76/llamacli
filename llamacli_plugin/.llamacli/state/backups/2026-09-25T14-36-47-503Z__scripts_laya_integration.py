...
def extract_answer(systemone_result: dict) -> str:
    ...
    if isinstance(routing, dict) and routing.get("model"):
        ...
        decision_reason = routing.get("reason")
    ...
    for qid, val in answers.items():
        ...
        if isinstance(val, dict):
            if "choice" in val:
                prob = val.get("probabilities", {}).get(val["choice"], 0.0)
                text = f"{val['choice']} (p={prob:.3f})"
            elif "score" in val:
                text = f"{val['score']:.2f}"
            elif "noul" in val:
                text = f"{val['noul']:.3f}"
