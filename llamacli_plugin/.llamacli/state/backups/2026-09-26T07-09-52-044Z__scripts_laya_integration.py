...
def cmd_fastcheck(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "short_circuit_only", False):
        return _emit_fastcheck_sc(result=_canned_sc_result(cfg), cfg=cfg)
...
def _emit_fastcheck_sc(*, result: dict, cfg: dict) -> int:
    """Print the SHORTCIRCUIT banner from a canned fast-check result.

    Used by the --short-circuit-only fast path (no live server). stdout line 1
    is exactly "SHORTCIRCUIT" so Node's /^short-circuit$/i matches; the banner
    text below mirrors cmd_gate()'s System-2 short-circuit output.
    """
    allow_sc, sc_reason = short_circuit_verdict(result, cfg, decision_qid="decision")
    if not allow_sc:
        # Still emit a SHORTCIRCUIT line so the gate can map it to skip:false;
        # Node treats anything other than an exact "SHORTCIRCUIT" first line as
        # skip. We prefix with SHORTCIRCUIT and carry the reason on line 2.
        print("SHORTCIRCUIT")
...
def _canned_sc_result(cfg: dict) -> dict:
    """Build a canned laya result that exercises short_circuit_verdict().

    For CI-safe --short-circuit-only fast checks we cannot call the server, so
    synthesize a verdict from config + the question text. The decision token is
    derived from the text: trivial tasks -> "yes" (allow-listed), otherwise
    leave it absent/ambiguous so short-circuit rejects when appropriate.
    """
    text = getattr(argparse.Namespace(), "_fastcheck_text", "") if False else ""  # placeholder
    return {}
