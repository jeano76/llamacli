...
def _decision_from_answer(val: dict) -> tuple[str, float, str]:
    ...
    # --- decision label -------------------------------------------------- #
    if "choice" in val:
        label = str(val["choice"])
        prob = float(val.get("probabilities", {}).get(label, 0.0))
        score = max(prob, 1.0 - prob)          # how strongly one side wins
        ...
        s = float(val["score"])                # laya-provided 0-1 intensity
        label = f"{s:.3f}"
        decision = "n/a"
    else:
...
def short_circuit_verdict(systemone_result: dict, cfg: dict,
    ...
    Returns (allow_short_circuit, reason). A short-circuit is allowed only when
    BOTH gating conditions hold, per §3.4 of the review:

      act_probability >= ACT_PROB_THRESHOLD  AND  confidence >= CONFIDENCE_THRESHOLD

    The gate requires agreement from both the model's action tendency and its
    own calibrated confidence; either falling below its bar means "reason with
    Ornith instead". When short-circuit is disabled this always returns False.
    """
    ...
    act_prob = float(answer.get("action", {}).get("act_probability", 0.0)) \
        if isinstance(answer.get("action"), dict) else 0.0
    conf = answer.get("answer_confidence") or answer.get("confidence")
    conf = float(conf) if isinstance(conf, (int, float)) else 0.0

    act_thr = float(cfg.get("actProbabilityThreshold", 0.85))
    conf_thr = float(cfg.get("confidenceThreshold", 0.7))

    if act_prob < act_thr:
        return False, f"act_probability {act_prob:.3f} < {act_thr}"
    if conf < conf_thr:
        return False, f"confidence {conf:.3f} < {conf_thr}"

    label, score, decision = _decision_from_answer(answer)
    return True, (f"short-circuit OK: decide '{label}' "
                  f"(act_prob={act_prob:.3f}, conf={conf:.3f})")
...
def cmd_gate(args: argparse.Namespace) -> int:
    ...
    questions = {"decision": _make_question(prompt, cfg)}
...
def cmd_fastcheck(args: argparse.Namespace) -> int:
    ...
    # When asked for a plain yes/no (decision), use the default noul question so
    # laya returns a `noul` float; otherwise honour the configured criteria.
    if not getattr(args, "criteria", None):
        questions = {"decision": _make_question(
            args.text, cfg, qid="decision")}
    else:
        base_q = _make_question(args.text, cfg)
        questions = {k: v for k, v in base_q.items()}
    ...
    if isinstance(answer, dict):
        label, score, decision = _decision_from_answer(answer)
        print(f"decision: {label}  (score={score:.3f}, conf={float(answer.get('answer_confidence') or answer.get('confidence') or score):.3f})")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_fc.add_argument("--question", default="decision", help="laya question id")
