...
export interface AgentLoopOptions {
   ...
   *  {skip:false} (or any error) to proceed normally with Ornith. The callback
   *  never throws — a disabled/errored gate must not break an ordinary turn. */
  layaGate?: (userText: string) => Promise<{ skip: boolean }>;
}
...
export class AgentLoop {
  ...
  async send(userText: string): Promise<void> {
    await this.enqueue(async () => {
      ...
      try {
        if (await this.opts.layaGate?.(userText)) {
