...
def short_circuit_verdict(systemone_result: dict, cfg: dict,
    ...
    allowed = SHORT_CIRCUIT_ALLOW_LIST or set(
        v.strip().lower() for v in cfg.get("allowList", [])) if (
            SHORT_CIRCUIT_ALLOW_LIST or cfg.get("allowList")) else None
    if not allowed:
        allowed = {"yes", "no", "proceed", "flag", "no_action"}
