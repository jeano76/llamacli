...
async function ensureSingleInstance() {
    ...
    if (others.length === 0)
        return;
    process.stdout.write(`이 프로젝트에서 llamacli가 이미 실행 중입니다 (PID ${others.join(", ")}).\n` +
 ...
 *  mechanism updates. */
async function maybeSelfUpdateAndRestart() {
    let binPath;
    try {
    ...
    if (!binPath.endsWith(".js"))
        return;
    const result = await checkAndApplyUpdate(binPath).catch((err) => ({ updated: false, reason: String(err?.message ?? err) }));
    if (!result.updated)
        return;
...
async function main() {
    ...
    const systemPrompt = injectSkillIndexIntoSystemPrompt(injectRulesIntoSystemPrompt(BASE_SYSTEM_PROMPT +
        (browserEnabled ? BROWSER_SYSTEM_PROMPT : "") +
        (config.enableThinking ? THINKING_LANGUAGE_SYSTEM_PROMPT : ""), rules), skillIndex);
    ...
    const loop = new AgentLoop({
        ...
        onAssistantDelta: (t) => globalThis.__llamacli_ui?.pushAssistantDelta(t),
        onAssistantDone: () => {
            globalThis.__llamacli_ui?.finalizeAssistant();
            globalThis.__llamacli_ui?.finalizeReasoning();
        },
        onReasoningDelta: (t) => globalThis.__llamacli_ui?.pushReasoningDelta(t),
        onQueueChange: (q) => globalThis.__llamacli_ui?.setQueue(q),
        onToolCall: (name, args) => {
            ...
            globalThis.__llamacli_ui?.pushTool(label);
        },
        onDiff: (_path, diff) => globalThis.__llamacli_ui?.pushDiff(diff),
        onToolResult: (command, output) => globalThis.__llamacli_ui?.pushToolResult(command, output),
        onStatus: (s) => globalThis.__llamacli_ui?.pushStatus(s),
        onContextUsage: (used, total) => globalThis.__llamacli_ui?.setContextUsedRatio(total > 0 ? Math.min(1, used / total) : 0),
        onPlanProgress: (done, total) => globalThis.__llamacli_ui?.setPlanProgress(done, total),
        onCompactionStatus: (status, timestamp) => globalThis.__llamacli_ui?.setCompactionStatus(status, timestamp),
        onCompactionDetail: (detail) => globalThis.__llamacli_ui?.pushCompactionDetail(detail),
        onTurnStart: () => globalThis.__llamacli_ui?.collapseDiffs(),
    });
    ...
    const exitAfterSaving = () => {
        const ui = globalThis.__llamacli_ui;
        ...
        Promise.race([
            save.catch((err) => ui?.pushStatus(`[couldn't save progress: ${summarizeErrorForDisplay(err.message)}] quitting anyway.`)),
            timeout,
        ]).finally(exitNow);
    };
    ...
    React.createElement(App, { cwd: projectRoot, model: config.model, startupBanner: { version: startupVersion(), repoUrl: REPO_URL }, initialHistory: initialHistory, onHistoryChange: (history) => {
        ...
        }, pendingResumeGoal: pendingCheckpoint?.goal ?? null, onResumeDecision: (resume) => {
            const ui = globalThis.__llamacli_ui;
            if (!resume) {
            ...
            loop
                ...
                .catch((err) => ui?.pushStatus(`[error] failed to resume from checkpoint: ${summarizeErrorForDisplay(err.message)}`))
                .finally(() => ui?.setBusy(false));
        }, onForceQuit: () => {
            // The "force" exit: no self-improvement-proposal gate (see
            // AppProps.onForceQuit) — just save and go.
            exitAfterSaving();
        }, onQuitWithoutSaving: exitNow, onSubmit: async (text) => {
            const ui = globalThis.__llamacli_ui;
            ui?.setBusy(true);
            try {
        ...
        }, onQueueMessage: (text) => loop.queueMessage(text), onSlashCommand: (key) => {
            const ui = globalThis.__llamacli_ui;
            switch (key) {
                case "quit": {
                    ...
                    loop
                        ...
                        .catch((err) => ui?.pushStatus(`[self-improvement analysis failed] ${summarizeErrorForDisplay(err.message)}`));
                    break;
                }
                case "help": {
                ...
                case "compact":
                    ui?.pushStatus(ui?.isBusy?.() ? "[compaction scheduled] It will run once the current turn finishes." : "[compaction started]");
                    ui?.setBusy(true);
                    loop
                        .forceCompact()
                        .catch((err) => ui?.pushStatus(`[compaction failed] ${summarizeErrorForDisplay(err.message)}`))
                        .finally(() => ui?.setBusy(false));
                    break;
                case "skills":
                    ui?.pushStatus(skillIndex.length
                        ? `Loaded skills:\n${skillIndex.map((s) => `- ${s.name}: ${s.trigger}`).join("\n")}`
                        : "No skills registered (.llamacli/skills/*.md).");
                    break;
                case "rules":
                    ui?.pushStatus(rules.length
                ...
                case "improve-apply":
                    loop
                        ...
                        .catch((err) => ui?.pushStatus(`[rule save failed] ${summarizeErrorForDisplay(err.message)}`));
                    break;
                case "plan-clear":
                    loop
                        .clearPlan()
                        .then(() => ui?.pushStatus("Plan progress cleared."))
                        .catch((err) => ui?.pushStatus(`[error] failed to clear plan: ${summarizeErrorForDisplay(err.message)}`));
                    break;
                // "queue" is handled locally inside App (needs the live queue state).
            }
        } }), { exitOnCtrlC: false });
    if (setupMessage)
        globalThis.__llamacli_ui?.pushStatus(setupMessage);
